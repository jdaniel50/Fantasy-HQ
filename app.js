...
function renderMatchupCards(matchupPairs, week) {
  ...
  matchupPairs.forEach(matchup => {
    ...
    team1Div.style.setProperty('--team-color-1', color1.headerLight);
    ...
    team2Div.style.setProperty('--team-color-2', color2.headerLight);
    ...
    // Analysis
    const analysis = generateMatchupAnalysis(matchup, week);
    if (analysis) {
      const analysisDiv = document.createElement('div');
      analysisDiv.className = 'matchup-analysis';
...
function generateMatchupAnalysis(matchup, week) {
  ...
  // Get remaining starters for both teams
  ...
  if (winningRemaining.length > 0) {
    // Winning team still has players
    const losingNames = formatPlayerList(losingRemaining);
    const winningNames = formatPlayerList(winningRemaining);
    analysisText = `${losingTeam} needs ${losingNames} to outscore ${winningNames} by ${deficit.toFixed(1)} points.`;
  } else {
    // Winning team is done
    if (losingRemaining.length > 0) {
      const losingNames = formatPlayerList(losingRemaining);
      analysisText = `${losingTeam} needs ${losingNames} to score ${deficit.toFixed(1)} points.`;
    }
  }
...
function advanceMatchupPresentation() {
  ...
  if (matchupPresentationStep < lastMatchupData.length) {
    ...
    const analysis = generateMatchupAnalysis(matchup, sleeperState?.week);
    if (analysis) {
      const analysisDiv = document.createElement('div');
      analysisDiv.className = 'matchup-analysis';
      analysisDiv.textContent = analysis;
      slide.appendChild(analysisDiv);
    }
  ...
  if (leagueInfo.hasDivisions) {
    ...
    divisions.forEach((divTeams, divId) => {
      divTeams.forEach((team, idx) => {
        ...
        const note = generateMagicNumber(team, divTeams, standing, leagueInfo, true);
        standings.push({
          ...team,
          standing,
          divisionId: divId,
    ...
    return sorted.map((team, idx) => {
      const standing = idx + 1;
      const note = generateMagicNumber(team, sorted, standing, leagueInfo, false);
      return {
        ...team,
        standing,
        note
...
function renderStandingsTable(standings, leagueInfo) {
  ...
  if (leagueInfo.hasDivisions) {
    ...
    divisions.forEach((teams, divId) => {
      const section = document.createElement('div');
      section.className = 'standings-section';
      
      const title = document.createElement('div');
      title.className = 'standings-section-title';
      title.textContent = `Division ${divId}`;
...
function createStandingsTable(teams) {
  ...
  teams.forEach(team => {
    ...
    teamName.style.setProperty('--team-color', colorMeta.headerLight);
    ...
    noteEl.className = 'standings-note';
    noteEl.textContent = team.note;
...
function advanceStandingsPresentation() {
  ...
  // Get all teams sorted by standing (1 to worst)
  const allTeams = [...lastStandingsData].sort((a, b) => a.standing - b.standing);
  ...
  if (standingsPresentationStep < allTeams.length) {
    const team = allTeams[standingsPresentationStep];
