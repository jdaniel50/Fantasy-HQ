...
function initializePreviousPowerRankings() {
  ...
  // For each league, check if previous rankings exist in localStorage
  // If not, initialize with the provided data
  Object.keys(previousRankings).forEach(leagueId => {
    const prevKey = `fantasy_power_prev_${leagueId}`;
    const existing = localStorage.getItem(prevKey);
    
    if (!existing) {
      // Convert team names to roster-based format
      // We'll need to match team names to roster IDs after leagues load
      // For now, store by team name temporarily
